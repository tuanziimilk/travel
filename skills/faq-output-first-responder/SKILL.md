...
- labels like `Answer:`
### 2. Upload-sheet mode
When the user requests upload-ready output, return structured values using the required field order. Follow `references/output-format.md` and the current project export contract. Do not rely on any `assets` template.
### 3. FAQ-row mode
When the user requests content-output rows, return structured row values using the required field order. Follow `references/output-format.md` and the current project export contract. Do not rely on any `assets` template.
## Core Decision Logic
### Use `Yes,` when:
...
- extra background
- marketing wording
- recommendations like 鈥渃heck the website鈥?- repeated details
- unsupported claims
...
- `Source`: fixed to `AI`
- `Subclass`: fixed to `first responder`
- `鏉垮潡鍚嶇О`: default to `faq`
