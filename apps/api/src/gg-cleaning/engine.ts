...
const FACT_FALLBACK_CLUES: Record<string, FactFallbackClues> = {
  ...
  "existing customer": {
    positive: ["existing customers receive", "repeat customer benefits", "annual pass", "beneficio por pertenecer", "alumnos de su escuela", "many magna returns"],
    negative: ["no public information", "no permanent loyalty program", "no existing customer discount"],
  },
  "loyalty program": {
    positive: ["treueprogramm", "program lojalnosciowy", "programa de fidelizacion", "membership program", "rewards program", "wilson club"],
    negative: [
      "no loyalty program",
  ...
  "new customer": {
    ...
    negative: ["no new customer discount", "keinen neukundenrabatt"],
  },
  child: {
  ...
  referral: {
    positive: ["refer a friend", "friend referral", "programa de patrocinio", "polec znajomemu", "친구 추천"],
    negative: ["no referral program", "keine freunde werben", "no formal friend referral"],
  },
};
...
const FACT_RULES: Record<string, FactRuleConfig> = {
  ...
  referral: {
    explicitTerms: [/\breferr(?:al|ing)?\b/i, /\brefer(?:-a-friend)?\b/i, /\bfriend referral\b/i, /\bpatrocinio\b/i, /\bpole(?:c|caj)\w*\b/i, /\b친구 추천\b/i, /\b초대 코드\b/i, /推荐好友/, /好友推荐/, /推荐折扣/, /推荐计划/, /推荐奖励/],
    positive: [
